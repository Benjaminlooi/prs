...
if(!isset($_SESSION['UID'])){ //if not login
  $_SESSION['prevPage'] = "admin";
  header("Location: ./login.php");
}
elseif($_SESSION['UType'] == "user"){ //if not admin
  ?>
  ...
  <body class="bg-light">
    <nav class="navbar navbar-light navbar-expand fixed-top" role="navigation">
      ...
      <div class="navbar-nav">
        ...
        elseif($_SESSION["UType"] == "admin"){
          ?>
          <a class="nav-link nav-item" href="./index.php">Home</a>
          <a class="nav-link nav-item active" href="./admin.php">All Pets</a>
          <?php
        }
    ...
    if(!$link){
      ...
      if(@$_POST["PetNameSearch"]){
        ...
        WHERE (`petName` LIKE '%".$petNameSearch."%')";
      }
      else{
        $queryGet = "select * from pet";
      }
      ...
      if(!$resultGet){
        die("Invalid query - get user's pet list: " . mysqli_error($link));
      }
      else{
        ?>
        <div class="view-top-content bg-light">
          <div class="container-fluid admin-view-container">
            <div class="row justify-content-center">
              <div class="col">
                ...
                elseif($_SESSION["admin_status"] == 2){ 
                  ?>
                  <div class="alert alert-success" role="alert">
                    Record has been set <span class="text-danger">Incomplete!</span>
                  </div>
                  <?php
                }
                ...
                if(@$_POST["PetNameSearch"]){
                  ...
                  ?>
                  <h2 class="float-left">Viewing all Pets</h2>
                  <?php
                }
                ?>
                <form action="admin.php" method="POST" class="form-inline float-right"> <!-- Search bar -->
                  ...
                  <tbody>
                    ...
                    while($row=mysqli_fetch_array($resultGet,MYSQLI_BOTH)){
                      ...
                      <tr>
                        ...
                        <td style="white-space:nowrap;"><?php if(!$row['petBreed']){ echo "<p class=\"text-danger empty-warning\">Not entered</p>";} else{ echo $row['petBreed']; } ?></td>
                        ...
                        <td style="white-space:nowrap;"><?php if($row['petWeight'] == 0){ echo "<p class=\"text-danger empty-warning\">Not entered</p>";} else{ echo $row['petWeight'] . "Kg"; } ?></td>
                        <td>
                        ...
                        <td class="text-center">
                          ...
                          if($row['status'] == 'Pending' || $row['status'] == 'Incomplete'){
                            ?>
                            <form  style='float:left; margin: 0 6px 8px 0' action='admin_process.php' method='POST'>
                              <input type='submit' name='action' value='Approve' class='btn btn-success'>
                              <input type='submit' name='action' value='Incomplete' class='btn btn-danger'>
                              <input type='hidden'name='petName' value='<?php echo $row['petName']; ?>'>
                            </form>
                            <?php
                          }
                          else{
                            echo "This data is already approved";
                          }
                          ?>" data-html="true">
